...
const ProfileForm = component$<FormProps>(
  ({ name, about, interests, onChange$ }) => {
    return (
      <Form class="flex max-w-xl flex-col gap-4">
...
const ProfileCard = component$<CardProps>(
  ({ name, about, interests, onEdit$ }) => {
    return (
      <div class="flex flex-col gap-10 md:flex-row md:items-center md:gap-20">
        ...
        <div class="order-first self-start md:order-last">
          <button
            onClick$={onEdit$}
            class="self-start rounded-full bg-white p-3 text-sm text-black shadow-[0_8px_15px_rgba(0,0,0,0.1)] transition-all hover:bg-[#90EE90] hover:text-white"
          >
            <svg
